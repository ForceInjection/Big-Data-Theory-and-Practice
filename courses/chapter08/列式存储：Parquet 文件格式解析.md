...
在压缩方面，行式存储的压缩效率较低。由于同一行中的不同列数据类型差异较大（如整数、字符串、日期等），压缩算法难以找到连续的重复模式，压缩比通常只有 2-4 倍。相比之下，列式存储中同一列的数据类型相同，压缩比可达 10-20 倍，显著降低了存储成本。
...
Parquet 的诞生源于实际生产环境中的迫切需求。Twitter 作为全球最大的社交媒体平台之一，每天需要处理数百 TB 的用户数据，面临着巨大的存储成本和查询性能挑战。Cloudera 作为 Hadoop 生态系统的领先企业，拥有深厚的大数据技术积累。两家公司的合作结合了实际业务需求和技术专长，与 **2013** 年共同开发出了 Parquet 这一革命性的列式存储格式。初期版本主要针对 Hadoop 生态系统优化，解决了 HDFS 上的列式存储需求。
...
- **传统 CSV 文件**：1GB 的销售数据
- **Parquet 格式**：压缩后仅需 150-250MB（压缩比 4-7:1）
- **查询速度**：比 CSV 快 5-10 倍
...
2. **核心技术优势**：深入认识了 Parquet 相比行式存储在查询性能、存储效率、压缩比等方面的根本性改进，包括 5-10 倍的查询性能提升和 70-90% 的存储空间节省。
...
| **压缩算法** | **压缩比**   | **压缩速度** | **适用场景**       |
| ------------ | ------------ | ------------ | ------------------ |
| **SNAPPY**   | 中等 (2-3x)  | 非常快       | 通用场景，平衡性能 |
| **GZIP**     | 高 (4-8x)    | 中等         | 存储敏感场景       |
| **LZO**      | 中等 (2-3x)  | 快           | 需要快速压缩       |
| **BROTLI**   | 很高 (6-10x) | 慢           | 极致压缩需求       |
...
| **算法**   | **压缩率**  | **速度** | **适用场景** |
| ---------- | ----------- | -------- | ------------ |
| **Snappy** | 中等 (2:1-4:1) | 很快     | 实时查询     |
| **GZIP**   | 高 (4:1-10:1)  | 慢       | 存储优先     |
| **ZSTD**   | 高 (3:1-8:1)   | 快       | 通用推荐     |
